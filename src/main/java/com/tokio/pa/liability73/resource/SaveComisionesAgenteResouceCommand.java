...
@Component(
		...
		property = {
				"javax.portlet.name=" + LiabilityQuotation73PortletKeys.LiabilityQuotation,
					"mvc.command.name=/RC/SaveComisionesAgente" 
				},
		service = MVCResourceCommand.class)

public class SaveComisionesAgenteResouceCommand extends BaseMVCResourceCommand{
	@Reference
	...
	protected void doServeResource(ResourceRequest resourceRequest,
		...
		String comisionesAg = ParamUtil.getString(resourceRequest, "comisionesAg");
		String infoCotiResponse = ParamUtil.getString(resourceRequest, "infoCotiResponse");
		
		JsonArray comisiones = gson.fromJson(comisionesAg, JsonArray.class);
		UbicacionesResponse resUbicaciones = getInfoUbicaciones(infoCotiResponse);
		
//		_CMServicesP3.guardaComisionesAgente(p_cotizacion, p_version, p_usuario, p_pantalla, p_comisiones);
		ComisionesResponse response = _CMServicesP3.guardaComisionesAgente(resUbicaciones.getCotizacion(), resUbicaciones.getVersion(), user.getScreenName(), LiabilityQuotation73PortletKeys.LiabilityQuotation, comisiones);
